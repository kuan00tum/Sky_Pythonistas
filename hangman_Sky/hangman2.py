...
while run:
    ...
    root.title('HANG MAN @BY SKY_PYTHONISTAS')
    root.config(bg = '#DFF705' )
    count = 0
    win_count = 0
    ...
    for i in range(0,len(selected_word)):
        x += 60
        exec('d{}=Label(root,text="_",bg="#DFF705",font=("arial",40))'.format(i))
    ...
    for q1 in button:
        exec('{}=Button(root,bd=0,command=lambda:check("{}","{}"),bg="#DFF705",activebackground="#DFF705",font=10,image={})'.format(q1[0],q1[1],q1[0],q1[1]))
        exec('{}.place(x={},y={})'.format(q1[0],q1[2],q1[3]))
        
    #hangman placement
    han = [['c1','h1'],['c2','h2'],['c3','h3'],['c4','h4'],['c5','h5'],['c6','h6'],['c7','h7']]
    for p1 in han:
        exec('{}=Label(root,bg="#DFF705",image={})'.format(p1[0],p1[1]))
    ...
    ex = Button(root,bd = 0,command = close,bg="#DFF705",activebackground = "#DFF705",font = 10,image = e1)
    ex.place(x=770,y=10)
    s2 = 'SCORE:'+str(score)
    s1 = Label(root,text = s2,bg = "#DFF705",font = ("arial",25))
    s1.place(x = 10,y = 10)
